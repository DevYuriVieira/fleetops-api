...
public sealed record CancelRouteCommand(Guid RouteId, string Reason);

public sealed class CancelRouteUseCase
{
    private readonly IRouteRepository _routeRepository;
    private readonly IUnitOfWork _unitOfWork;

    public CancelRouteUseCase(IRouteRepository routeRepository, IUnitOfWork unitOfWork)
    {
        _routeRepository = routeRepository;
        _unitOfWork = unitOfWork;
    }

    public async Task<RouteDto> ExecuteAsync(CancelRouteCommand command, CancellationToken cancellationToken = default)
    {
